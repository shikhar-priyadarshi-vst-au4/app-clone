...
const getRatingsFormat = (ratings) => {
  let rateLen = ratings?.length;
  return (
    <>
      {rateLen &&
        ratings.map((data, i) => (
          <div className="rate-container">
            <span className="rate-badge">
              <div className="badge-num">{data?.rate ?? "n/a"}</div>
              <svg
            ...
            <span className="rate-category">{data?.category}</span>
            {i !== rateLen - 1 && <span className="rate-breaker">|</span>}
          </div>
        ))}
    </>
  );
...
const applydataLimiter = (data, limiter) => data?.join(`${limiter}`);
    ...
    <>
      <div className="card-cover">
        ...
        <div className="card-holder">
          ...
          <div className="card-ratings">{getRatingsFormat(props?.ratings)}</div>
          <p className="card-detail">{applydataLimiter(props?.detail, ", ")}</p>
          <p className="card-loca">{applydataLimiter(props?.location, ", ")}</p>
        </div>
      </div>
    </>
  );
